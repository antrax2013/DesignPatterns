...
### 2.4. Decorator
Le décorateur est un pattern dont l'objectif est d'étendre dynamiquement le comportement, ou les données, d'un objet sur le principe des poupées russes (⚠️ SRP). 

### 2.5. Façade
La façade est un pattern dont l'objectif est de fournir une interface simple pour agréger des données provenant de différentes sources. L'idée étant de masquer la complexité du système et de fournir une interface plus facile à utiliser.

### 2.6. Flyweight
Le flyweight, ou poids mouche, est un pattern dont l'objectif est de réduire la consommation de mémoire en partageant des données communes entre plusieurs objets.

### 2.7. Proxy
Le proxy, ou mandataire, est un pattern dont l'objectif est de contrôler l'accès à un objet en interposant un objet intermédiaire. 
